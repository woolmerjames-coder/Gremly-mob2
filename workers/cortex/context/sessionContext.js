...
// ============================================================================
// DATE HELPER (server-side UTC dates for queries)
// ============================================================================
...
async function queryWeekSummary(supabaseUrl, headers, userId) {
  ...
  try {
    ...
    const createdWeek = todos.filter(
      (t) => !t.archived && new Date(t.created_at) > new Date(weekAgo),
    ).length;

    const completedWeek = todos.filter(
      (t) => t.completed_at && new Date(t.completed_at) > new Date(weekAgo),
    ).length;

    const stuckCount = todos.filter(
      (t) => !t.archived && !t.completed_at && (t.sweep_reschedule_count || 0) >= 3,
    ).length;
...
async function queryHabitHealth(supabaseUrl, headers, userId) {
  try {
    ...
    const weekAgoStr = formatDateOnly(weekAgo);
...
async function queryUpcomingMilestones(supabaseUrl, headers, userId) {
  try {
    ...
    const thirtyDaysStr = formatDateOnly(thirtyDays);
