 ...
 **DECISION TREE:**
 ...
 Return ONLY valid JSON:
 {
  ...
  === CRITICAL GATES (CHECK FIRST — Before ANY split decision) ===

These three tests MUST be applied before evaluating split signals. If ANY gate returns YES → SINGLE. Do not proceed to split evaluation.

**GATE 1: SAME SUBJECT TEST**
Are all parts of this drop about THE SAME underlying topic, situation, activity, or decision?

- Multiple emotions about one situation → ONE journal (the situation is the subject)
- One activity on multiple days → ONE habit (the activity is the subject)
- Options or alternatives for one choice → ONE item (the decision is the subject)
- Planning details for one thing → ONE item (the thing is the subject)

Ask: "What is the NOUN at the center of this?" If there's only one central noun/topic, it's SINGLE.

**GATE 2: EMOTIONAL UNITY TEST**
Is the user processing feelings, even if they mention multiple emotions or a coping response?

- Conflicting emotions about one thing → ONE journal ("anxious but excited about the presentation")
- Emotion + coping action → ONE journal ("stressed, need to take a walk")
- Venting with narrative flow → ONE journal ("bad day, client yelled, boss took their side")

Emotional processing is inherently unified. The value is in the expression as a whole.

**GATE 3: FREQUENCY UNITY TEST**
Is the user describing ONE recurring activity with schedule details?

- One activity + multiple day names → ONE habit ("run on Tuesdays and Thursdays" = one running habit)
- One activity + frequency words → ONE habit ("meditate morning and evening" = one meditation habit)

Day names and time slots are SCHEDULE DETAILS for a single habit, not separate habits.

**THE "BUT ALSO" TRAP:**
Words like "but also", "and also", "also" do NOT automatically mean split. These words connect RELATED thoughts as often as unrelated ones. Apply the three gates first. Only split if the gates all pass AND subjects are genuinely different.

=== TOP-DOWN EVALUATION ===

After passing the gates, read the ENTIRE drop and ask:
