              ...
              actions.map(async (action: CortexAction) => {
                try {
                  if (action.type === 'add.to.list') {
                    ...
                    // Use LLM due if present; else deterministically parse
                    let due = action.payload.due ?? null;
                    let undefinedDue = !due;
                    let parsedDueExplain: string | undefined;

                    if (!due) {
                      const parsedDue = parseDue(trimmed);
                      parsedDueExplain = parsedDue.explain;
                      if (parsedDue.iso && parsedDue.confidence >= 0.9) {
                        due = parsedDue.iso;
                        undefinedDue = false;
                      }
                    }
