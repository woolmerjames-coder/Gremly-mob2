...
jest.mock('../hooks/useUnifiedOverlayController', () => ({
  useUnifiedOverlayController: () => ({ close: jest.fn(), openCreate: jest.fn() }),
}));
...
describe('Mind Drop uniqueness when converting note to todo', () => {
  beforeEach(() => {
    ...
    mockRepo.create.mockResolvedValue({ id: existingRecord.id, type: 'note' });
    mockRepo.update.mockResolvedValue({ id: existingRecord.id, type: 'todo' });
    mockRepo.findBySourceMessageId.mockImplementation((type: string) => {
      if (type === 'todo') return Promise.resolve(existingRecord);
